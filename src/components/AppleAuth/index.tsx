...
const AppleAuth = ({ onSuccess, loading }: { onSuccess: (user: any) => void, loading: boolean }) => {
    const onAppleButtonPress = async () => {
        try {
            ...
            if (!fullName || !fullName.givenName) {
                userName = `Apple User ${appleAuthRequestResponse.user.substring(0, 8)}`;
            }
            else {
    ...
    return (
        <TouchableOpacity style={styles.appleButton} onPress={onAppleButtonPress}>
            <View style={styles.buttonContent}>
                <Icon name="apple" size={20} color="#000" iconStyle="brand" style={styles.appleIcon} />
