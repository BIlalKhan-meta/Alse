import { FlatList, Image, ImageSourcePropType, Modal, StyleProp, StyleSheet, Text, TextInput, TextStyle, TouchableOpacity, View, ViewStyle } from 'react-native';
...
const ReactModal: React.FC<ReactModalProps> = (props) => {
    ...
    const [selectedTab, setSelectedTab] = useState<'all' | 'heart' | 'like'>('all');

    // Example counts (replace with actual data)
    const allCount = reactions.length;
    const heartCount = reactions.filter(reaction => reaction.reactionType === 'heart').length;
    ...
    const renderReactionItem = ({ item }: { item: Reaction }) => (
        <>
            <View style={styles.reactionItem} key={item.id}>
                ...
                <InterMedium style={styles.reactionUserName}>{item.userName}</InterMedium>
            </View>
            <HorizontalSeparator />
        ...
        <>
            <Modal
                ...
                <View style={styles.container}>
                    <InterBold style={styles.heading}>People Who Reacted</InterBold>
                    <View style={styles.header}>
                        <TouchableOpacity style={[styles.tabButton, selectedTab === 'all' && styles.activeTab]} onPress={() => setSelectedTab('all')}>
                            <Text style={[styles.tabButtonText, selectedTab === 'all' && styles.activeTabTxt]}>All ({allCount})</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={[styles.tabButton, selectedTab === 'heart' && styles.activeTab]} onPress={() => setSelectedTab('heart')}>
                            <Text style={[styles.tabButtonText, selectedTab === 'heart' && styles.activeTabTxt]}>Heart ({heartCount})</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={[styles.tabButton, selectedTab === 'like' && styles.activeTab]} onPress={() => setSelectedTab('like')}>
                            <Text style={[styles.tabButtonText, selectedTab === 'like' && styles.activeTabTxt]}>Like ({likeCount})</Text>
                        </TouchableOpacity>
                    </View>
