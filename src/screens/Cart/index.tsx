...
const Cart = () => {
  ...
  const handleChange = async (label: string, id: number) => {
    if (!cartData?.carts?.data) return;

    let index = cartData.carts.data.findIndex((item: any) => item.id == id);
    if (index === -1) return;
    ...
    const form = new FormData();
    form.append('quantity', arr[index].quantity.toString());
    await updateCartItem(form, id);
  };
  ...
  const getData = async () => {
    ...
    try {
      ...
      if (res?.data?.data) {
        ...
        setCartData(null);
        setApiError('No cart data available');
      }
    } catch (error) {
      console.log('Error fetching cart data:', error);

      // If it's a 500 error (backend issue), show specific error message
      if ((error as any)?.response?.status === 500) {
        console.log('Backend error detected - showing error state');
        setApiError(
          'Backend error: Unable to load cart data. Please try again later.',
        );
      } else {
        setApiError('Failed to load cart data. Please try again.');
      }
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      ...
      title: `Cart (${cartData?.carts?.data?.length || 0})`,
    });
  }, [navigation, cartData]);
          ...
          <FlatList
            ...
            renderItem={({item, index}) => (
              <CartItem
                ...
                onDelete={handleDelete}
                showSeparator={
                  index !== (cartData?.carts?.data?.length || 0) - 1
                }
                showDelete={true}
              />
            )}
        ...
        {cartData?.carts?.data?.length > 0 && (
          <>
            <Summary
              ...
              deliveryCharges={cartData?.total_delivery_Fees || 0}
              style={{}}
              titleStyle={{}}
            />
            ...
            <CustomButton
              ...
              txtstyle={styles.shoppingTxt}
              onPress={() =>
                (navigation as any).navigate('MarketPlaceNavigation', {
                  screen: 'Marketplace',
                })
              }>
              Continue Shopping
